...
The system SHALL store each animation as a directory of individually-numbered PNG files rather than a single container file.

#### Scenario: Frame files written to disk on generation
- **WHEN** animation generation completes successfully
- **THEN** the output directory SHALL contain sequentially-numbered PNG files, one per frame

#### Scenario: Existing frame sequence detected on load
- **WHEN** the Results view loads for a scenario that already has a generated animation
...
The system SHALL allow the user to export a generated animation as an Animated PNG file.

#### Scenario: Export as APNG succeeds
- **WHEN** the user activates "Export as APNG" for a done animation card
- **THEN** the system SHALL assemble all frames into an APNG file and write it to the scenario's output directory
...
The system SHALL allow the user to export a generated animation as an MP4 file when ffmpeg is available on the system.

#### Scenario: Export as MP4 with ffmpeg available
- **WHEN** the user activates "Export as MP4" and ffmpeg is installed on the system
- **THEN** the system SHALL invoke ffmpeg with the frame sequence as input and write an MP4 file to the scenario's output directory
