...
const NUMBER_OF_EPOCHS: usize = 20_000;
...
#[test]
#[ignore]
fn heavy_sensor_number() {
    let base_id = "Sensor Number";
    let path = Path::new(COMMON_PATH);

    let number_of_sensors = vec![1, 4, 8, 16, 32, 48, 64];
    let trials = 10;

    create_and_run(number_of_sensors, trials, base_id, path);
}
    ...
    for number_of_sensors in number_of_sensors {
            ...
            for scenario in scenarios {
                    ...
                    for (i, delay) in delays.iter().enumerate() {
                        let writer = BufWriter::new(
                            File::create(path.join(format!(
                                "num: {number_of_sensors}, trial {trial} delay: {i}.npy",
                            )))
                            .unwrap(),
                        );
                ...
                line_plot(
                    ...
                    Some(&path.join(format!(
                        "num_{number_of_sensors:03}_trial_{trial}_delays.png"
                    ))),
                ...
                line_plot(
                    ...
                    delays_error,
                    Some(&path.join(format!(
                        "num_{number_of_sensors:03}_trial_{trial}_delays_error.png"
                    ))),
        ...
        if SAVE_NPY {
            ...
            for (label, loss) in labels.iter().zip(losses.iter()) {
                let writer = BufWriter::new(
                    File::create(
                        path.join(format!("loss - num_ap: {number_of_sensors} {label}.npy")),
                    )
                    .unwrap(),
                );
        ...
        log_y_plot(
            Some(&x_epochs),
            losses,
            Some(&path.join(format!("num_{number_of_sensors:03}_loss.png"))),
...
#[tracing::instrument(level = "trace")]
fn create_and_run(number_of_sensors: Vec<i32>, trials: usize, base_id: &str, path: &Path) {
    ...
    for number_of_sensors in &number_of_sensors {
        for trial in 0..trials {
            let id = format!("{base_id} - Num: {number_of_sensors:03}, trial: {trial:03}");
    ...
    plot_results(path, base_id, &scenarios, number_of_sensors, trials);
}
