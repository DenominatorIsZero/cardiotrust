...
use systems::{
    ...
    handle_color_mode_button, handle_color_mode_option_click, handle_control_action_buttons,
    handle_plot_click, handle_plot_collapse_button, handle_plot_resize, handle_section_tab_click,
    handle_toolbar_buttons, log_camera_pose_on_f3, poll_screenshot_dialog,
    sync_overlay_panel_contents, sync_plot_visuals, sync_volumetric_layout,
...
use types::{PlotImageState, ScreenshotDialogReceiver, VolumetricViewState};
...
impl Plugin for VolumetricViewPlugin {
    ...
    fn build(&self, app: &mut App) {
        let volumetric_condition = in_state(UiState::Volumetric);

        app.init_resource::<VolumetricViewState>()
            .init_resource::<PlotImageState>()
            ...
            .add_systems(OnEnter(UiState::Volumetric), spawn_volumetric_view)
            .add_systems(OnExit(UiState::Volumetric), despawn_volumetric_view)
            .add_systems(
                Update,
                (
